...
def find_episode_by_title_card(
    ...
    Args:
        ...
        match_threshold: Minimum similarity ratio for episode matching (default: 0.55)
        return_details: If True, return (media_file, episode_name, timestamp, extracted_text) (default: False)
        max_dimension: Maximum width or height in pixels for frame scaling (default: None = full resolution)
                      Higher values improve OCR accuracy but slow down processing
                      None = use full video resolution (best accuracy, slowest)
    ...
    try:
        result = _find_episode_sequential(
            media_file, episode_names, match_threshold, return_details, max_dimension,
...
def _find_episode_sequential(
    ...
    try:
        ...
        if pre_extracted_frames is not None:
            # Use pre-extracted frames from background extraction
            ...
            frames = extract_frames_batch(
                ...
                duration=duration,  # Pass duration limit to frame extraction
                hwaccel=hwaccel  # Pass hardware acceleration setting
            )
        
        ffmpeg_time = time.time() - ffmpeg_start
        if enable_profiling:
            logger.info(f"FFMPEG extraction completed in {ffmpeg_time:.2f}s ({len(frames)} frames)")
            # Note: Frames are scaled to max_dimension by FFMPEG if max_dimension is set
        
        # Process frames individually to avoid memory issues
        # Batch processing has memory calculation bugs in EasyOCR that cause excessive memory usage
        # Processing one at a time uses minimal memory (~1 MB per frame) and is actually faster
        ocr_start = time.time()
        ...
        for timestamp, frame_bytes in frames:
            ...
            # Extract text from frame individually using vanilla EasyOCR settings
            extracted_text = extract_text_from_frame(frame_bytes)
            
            if not extracted_text:
                continue
            
            # Try to match against episode names
            ...
            if matched_episode:
                ...
                # Found a match, log timing and return immediately
